...
export function generateMarkdown(config: ProjectConfig, s: AppStrings): string {
  ...
  if (config.techStack.length > 0) {
    ...
    lines.push("");
    lines.push(
      `| ${s.mdTechCategory} | ${s.mdTechTechnology} | ${s.mdTechVersionNotes} |`,
    );
    lines.push("|----------|------------|-----------------|");
  ...
  if (config.coreFeatures.length > 0) {
    ...
    lines.push("");
    for (const f of config.coreFeatures) lines.push(`- ${f}`);
  }
  ...
  if (config.acceptanceCriteria.length > 0) {
    ...
    lines.push("");
    for (const c of config.acceptanceCriteria) lines.push(`- ${c}`);
  }
  ...
  if (config.whatNotToDo.length > 0) {
    ...
    lines.push("");
    for (const w of config.whatNotToDo) lines.push(`- ${w}`);
  }
